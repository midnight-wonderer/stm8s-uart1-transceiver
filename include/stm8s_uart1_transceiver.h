...
#define stm8s_uart1_transceiver__setup_clock(brr1, brr2) \
  do {                                                   \
    UART1->BRR2 = (brr2);                                \
    UART1->BRR1 = (brr1);                                \
  } while (0)
...
typedef struct _stm8s_uart1_transceiver__receiver_config {
  void (*data_received)(uint8_t data);
  void (*overrun_detected)(void);
  void (*data_error_detected)(void);
} stm8sUart1Transceiver_ReceiverConfig;

void stm8s_uart1_transceiver__setup_receiver(
    stm8sUart1Transceiver_ReceiverConfig* config);
#endif

#ifndef STM8S_UART1_DISABLE_TRANSMITTER

void stm8s_uart1_transceiver__setup_transmitter(void);

#define stm8s_uart1_transceiver__is_ready() (UART1->SR & UART1_SR_TXE)

#define stm8s_uart1_transceiver__send(data) \
  do {                                      \
    UART1->DR = (data);                     \
  } while (0)
#endif
